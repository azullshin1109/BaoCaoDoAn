...
int LSPD(int cp, int inEps[], int isEvent[]){
    double d[MAXV];
    int inQ_LSPD[MAXV];//hang doi 
    int Q_LSPD[MAXV];
    int head = 0, tail = 0;
    
	//Khoi tao d(cp)=0, d() = INF, day cp vao hang doi 
    for (int i = 0; i < V; i++) {
        d[i] = INF;
        inQ_LSPD[i] = 0;
        inEps[i] = 0;
...
//Chen thu tu giam dna
void pushByDensity(int x, int Q[], int &head, int &tail){
    int i = tail - 1;
		//Phan tu trong Q co mat do nho hon -> day ra sau
